...
# You can place autoredis in your path and set "autoredis_directory"
# to the path of the autoredis folder
# export autoredis_directory=""
...
if [ ! -z "$autoredis_directory" ]; then
  cd $autoredis_directory
fi

if [ ! -d "${deployment_base}/common/keys" ]; then
  mkdir -p "${deployment_base}/common/keys"
  chmod 750 "${deployment_base}/common/keys"
  echo "Add your private keys ending in '.pem' to \"${deployment_base}/common/keys\" directory." 
  exit 2
...
Usage:
  autoredis.sh <deployment_name> [create|info|all] | [terraform|dns|cluster] ...

1. Run './autoredis.sh <deployment> create' to create a deployment directory
